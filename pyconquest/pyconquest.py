...
class pyconquest:
    ...
    def __init__(
        self,
        data_directory="data",
        sql_inifile_name="dicom.sql",
        database_filename="conquest.db",
        ...
        if connect_and_read_sql:
            ...
            self.connect_db()
        log.info(
            "Created pyconquest({}) with dir:({}); db:({});ini:({}) ".format(
                self.__version__,
                data_directory,
                database_filename,
                sql_inifile_name,
    ...
    def connect_db(self):
        ...
        result = self.execute_db_query(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='DICOMseries' "
        )
        if not result:
    ...
    def close_db(self):
        ...
        log.info(
            "Time elapsed : {:.1f} seconds ({:.2f} min)".format(
                float(timediff), float(timediff / 60.0)
            )
        )
    ...
    def execute_db_query(self, query, return_list_from_col=None):
        ...
        try:
            ...
            log.error(
                "exception "
                + str(e)
                + "\nencountered in execution of db query: "
                + query
            )
    ...
    def insert_dict(
        ...
        datadict,
        exceptions={},
        default_format="character varying(128)",
    ):
        """
        ...
        try:
            ...
            if str(e).startswith("no such table") == True:
                ...
                log.error(
                    "Error {} when inserting dict {} into table {}".format(
                        str(e), datadict, tablename
                    )
                )
    ...
    def write_tags(self, ds, filename="", check_existing=True):
        ...
        if check_existing:
            row_exists = self.__check_if_table_contains(
                "DICOMimages", "SOPInstanc", sopinstanceuid
            )
        else:
            row_exists = False
        ...
        if not row_exists:
            ...
            updatequery = """update DICOMimages set DatabaseTimeStamp=\'{}\' where SOPInstanc=\'{}\'""".format(
                time.time(), sopinstanceuid
            )
        ...
        if not (seriesuid == self.__prev_seriesuid):
            ...
            if not self.__check_if_table_contains(
                "DICOMseries", "SeriesInst", seriesuid
            ):
            ...
            if not (studyuid == self.__prev_studyuid):
                ...
                if not self.__check_if_table_contains(
                    "DICOMstudies", "Studyinsta", studyuid
                ):
    ...
    def update_table(self, ds, extra_cols=None):
        """
    ...
    def rebuild_database_from_dicom(
        ...
        already_present = self.execute_db_query(
            query="select PatientID from dicompatients",
        ...
        for root, dirs, files in os.walk(directory, topdown=True):
            ...
            for name in files:
                ...
                try:
                    ...
                    self.write_tags(
                        ds,
                        full_filename[string_startingpoint:],
    ...
    def store_dicom_file(self, filename, remove_after_store=False):
        ...
        try:
            ...
            log.info(
                "stored file : {} in database at location: {}".format(
    ...
    def store_dicom_files_from_directory(
        self, directory_name, remove_after_store=False
    ):
        ...
        if os.path.exists(directory_name):
            for root, dirs, files in os.walk(directory_name, topdown=True):
                for name in files:
                    full_filename = os.path.join(root, name)
                    log.info("Processing ... " + full_filename)
                    self.store_dicom_file(
                        full_filename, remove_after_store=remove_after_store
                    )
        ...
        if dicomtype == "RTSTRUCT":
            ...
            if self.__compute_hash:
                ...
                returndict["hash"] = hashlib.md5(
                    pickle.dumps(ds[0x3006, 0x0039].value)
                ).hexdigest()
            ...
            if self.__compute_hash:
                returndict["hash"] = hashlib.md5(
                    pickle.dumps(ds[0x300A, 0x00B0].value)
                ).hexdigest()
            ...
            try:
                ...
                log.error(
                    "Error when extracting referenced_seriesuid of RTSTRUCT from RTPLAN file"
                )
            ...
            try:
                ...
                log.error(
                    "Error when extracting referenced_seriesuid of RTPLAN from RTDOSE file"
                )
    ...
    def delete_series(self, seriesuid="", query=None, delete_files=False, mrn=None):
        ...
        if mrn is not None:
            if delete_files == True:
                ...
                log.info(
                    "Deleted all database entries for patient {} from the database tables".format(
                        mrn
                    )
                )
                return 1
        ...
        # single seriesuid given
            ...
            file_query = 'select ObjectFile,ImagePat,dicomseries.StudyInsta from dicomimages \
                            ...
                            where dicomimages.seriesinst=="{}"'.format(
                seriesuid
            )
            ...
            for row in return_list:
                ...
                if os.path.exists(filename):
                    if delete_files:
                        ...
                        log.info(
                            "not deleting file, only DB entries,  since delete_files=False".format(
                                filename
                            )
                        )
                else:
                ...
                query = 'delete from dicomimages where ObjectFile="{}"'.format(
                    row["ObjectFile"]
                )
                self.execute_db_query(query)
            ...
            if remaining_series[0]["nr"] == 0:
                ...
                self.execute_db_query(query)
                log.info(
                    "no more series : now deleting studiuid entry in db : {}".format(
                        studyuid
                    )
                )
            ...
            if remaining_studies[0]["nr"] == 0:
                ...
                self.execute_db_query(query)
                log.info(
                    "no more studies : now deleting patient db entry with number : {}".format(
                        patientid
                    )
                )
    ...
    def copy_dicom_files_to_dest(
        ...
        if not seriesuid is None:
            if isinstance(seriesuid, list):  # recursive call in case of list as input
                ...
                file_query = 'select ObjectFile,ImagePat from dicomimages where seriesinst="{}"'.format(
                    seriesuid
                )
                return_list = self.execute_db_query(file_query)
    ...
    def send_dicom(
        ...
        filename_list = []
        if not patientid == "":
            query = "Select ObjectFile from DICOMimages where imagepat='{}'".format(
                patientid
            )
        elif not seriesuid == "":
            query = "Select ObjectFile from DICOMimages where seriesinst='{}'".format(
                seriesuid
            )
        elif not query == "":
    ...
    def start_dicom_listener(self, port=5678, write_to_database=True):
        ...
        if write_to_database is True:
            ...
            print(
                "Not updating the database when receiving data (write_to_database was set to False !)"
            )

        handlers = [
            (evt.EVT_C_STORE, self.handle_dicom_store_request),
    ...
    def handle_dicom_store_request(self, event):
        ...
        filename = "{}/{}/{}.dcm".format(
            self.data_directory, patientid, ds.SOPInstanceUID
        )
        path = "{}/{}".format(self.data_directory, patientid)
        if not os.path.exists(path):
            os.makedirs(path)
    ...
    def set_roi_filter(
        self, exclude=[""], include=[""], roi_filter_flags=re.IGNORECASE
    ):
        ...
        log.info(
            "Filtervalues :  include: {} ; exclude {} ; flags : {}".format(
                self.include_filter,
                self.exclude_filter,
    ...
    def __create_db_index_tables(self):
        ...
        index_dicomimages = 'CREATE INDEX IF NOT EXISTS "index_dicomimages" ON "DICOMimages" ("SOPInstanc")'
        index_dicomseries = 'CREATE INDEX IF NOT EXISTS "index_dicomseries" ON "DICOMseries" ("SeriesInst")'
    ...
    def __set_default_database(self):
        log.info(
            "Using default database layout,specify .sql file during instance creation to change this"
        )
        self.__db_design = {
            "DICOMpatients": [
