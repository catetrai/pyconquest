...
# normal rebuild
c = pyconquest(
    sql_inifile_name="dicom.sql", loglevel="INFO", compute_hash=True
)
c.add_column_to_database(
    tablename="DICOMpatients",
    column_definition=["0x0020", "0x000d", "StudyInst"],
)
...
c.copy_dicom_files_to_dest(
    seriesuid="1.2.840.113704.1.111.7700.1448024597.12",
    destination="data2",
...
c.copy_dicom_files_to_dest(
    query=query, destination="data2", UseSubDirectories=True
)
...
c.send_dicom(
    seriesuid="2.16.840.1.113669.2.931128.880152.20190524082318.537836"
)
...
all_series = [
    "2.16.840.1.113669.2.931128.880152.20190524082318.537836",
    "2.16.840.1.113669.2.931128.880152.20190524082317.937233",
    "1.2.840.113704.1.111.7700.1448024597.12",
...
print(
    "after filtering ptv out here should only be lung : "
    + str(c.filter_roinames(["ex", "lung"]))
)
c.set_roi_filter(include=["PTV"], exclude=[""], roi_filter_flags=re.IGNORECASE)
print(
    "after filtering PTV in (include) there should only be ptv : "
    + str(c.filter_roinames(["ex", "lung", "ptv"]))
)
c.set_roi_filter(include=["PTV"], exclude=[""], roi_filter_flags=0)
print(
    "after filtering PTV in (include) without ignorease there should only be only PTV not ptv : "
    + str(c.filter_roinames(["ex", "PTV", "ptv"]))
)
c.set_roi_filter(include="PTV", exclude=[""], roi_filter_flags=0)
print(
    "after filtering PTV in (include) without ignorease there should only be only PTV not ptv : "
...
lst = c.execute_db_query(
    "select * from dicomseries", return_list_from_col="SeriesInst"
)
